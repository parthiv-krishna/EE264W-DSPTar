...
% distortion_out = distortion_in./(1 + abs(distortion_in));
distortion_out = distortion_in + distortion_in.^3 / 3 + distortion_in.^5 / 5;
...
% FFT
f_sin = (1:N)/N * Fs;
db = 20*log10(abs(fft(y_sin)));
db_dist = 20*log10(abs(fft(y_sin_dist)));
...
plot(f_sin, db_dist);
legend("Input", "Output");
title("Sample distorted sine wave FFT");
hold off;
